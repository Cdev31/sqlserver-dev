...
(NEWID(), @project2_id, @mit_id, (SELECT call_id FROM funding_calls WHERE name = 'Quantum Initiative'), 'CONTRACT-002', 'Quantum computing for pharmaceuticals', 'mixed', 2500000.00, 'USD', '2024-04-30', '2024-05-15', '2027-05-14', 'active'),
(NEWID(), @project3_id, @stanford_id, (SELECT call_id FROM funding_calls WHERE name = 'Cancer Research Program'), 'CONTRACT-003', 'Personalized cancer treatment research', 'mixed', 1800000.00, 'USD', '2024-02-28', '2024-03-01', '2028-02-28', 'active'),
(NEWID(), @project4_id, @maxplanck_id, (SELECT call_id FROM funding_calls WHERE name = 'Quantum Initiative'), 'CONTRACT-004', 'Quantum encryption systems development', 'mixed', 950000.00, 'EUR', '2024-05-31', '2024-06-01', '2026-11-30', 'active'),
...
(NEWID(), @project6_id, @nih_id, (SELECT call_id FROM funding_calls WHERE name = 'Cancer Research Program'), 'CONTRACT-006', 'Antiviral therapies development', 'mixed', 2200000.00, 'USD', '2024-01-31', '2024-02-01', '2027-01-31', 'active'),
...
VALUES
(NEWID(), 'AI System for Automated Cancer Diagnosis', 'US20240012345', 'An AI-powered diagnostic system for early cancer detection from medical imaging.', '2024-01-05', NULL, 'USA', 'USPTO', 'applied', @project1_id),
(NEWID(), 'Quantum Algorithm for Molecular Simulation', 'EP20240067890', 'Quantum algorithm enabling efficient molecular simulations for drug discovery.', '2024-02-12', NULL, 'EU', 'EPO', 'applied', @project2_id),
(NEWID(), 'Personalized Immunotherapy Method', 'US20240054321', 'A method to personalize cancer immunotherapy using genetic profiling.', '2024-01-20', NULL, 'USA', 'USPTO', 'applied', @project3_id),
(NEWID(), 'Quantum Encryption Device', 'DE20240098765', 'Device implementing quantum entanglement for secure communications.', '2024-03-01', NULL, 'Germany', 'DPMA', 'applied', @project4_id),
(NEWID(), 'Neural Language Processing Chip', 'US20240019283', 'Hardware accelerator chip for deep learning in NLP tasks.', '2024-02-25', NULL, 'USA', 'USPTO', 'applied', @project5_id),
(NEWID(), 'Broad-Spectrum Antiviral Compound', 'US20240037465', 'Novel compound for treatment of multiple viral pathogens.', '2024-01-15', NULL, 'USA', 'USPTO', 'applied', @project6_id),
(NEWID(), 'Bayesian Climate Prediction Model', 'UK20240065432', 'Bayesian hierarchical model for advanced climate prediction.', '2024-03-05', NULL, 'UK', 'UKIPO', 'applied', @project7_id),
(NEWID(), 'CRISPR-Cas9 Gene Therapy Kit', 'CH20240082347', 'Kit for precise CRISPR-Cas9 based treatment of rare diseases.', '2024-02-18', NULL, 'Switzerland', 'IPI', 'applied', @project8_id),
(NEWID(), 'Autonomous Surgical Robot', 'US20240094512', 'A robotic system capable of autonomous minimally invasive surgeries.', '2024-01-28', NULL, 'USA', 'USPTO', 'applied', @project9_id),
(NEWID(), 'Graphene Nanocomposite for Energy Storage', 'JP20240018765', 'Graphene-based nanocomposite material for high-capacity energy storage.', '2024-02-10', NULL, 'Japan', 'JPO', 'applied', @project10_id);

-- 20. Insertar inventores de patentes
INSERT INTO patent_inventors (researcher_id, patent_id, inventor_order, contribution)
-- inventor principal = principal_researcher_id del proyecto
...
UNION ALL
-- inventor secundario = según proyecto
SELECT 
    CASE 
...
VALUES
(NEWID(), @sarah_id, 'Artificial Intelligence, Medical Imaging', 'Harvard University', 'active'),
(NEWID(), @michael_id, 'Quantum Computing, Algorithms', 'MIT', 'active'),
(NEWID(), @emily_id, 'Oncology, Immunotherapy', 'Stanford University', 'active'),
(NEWID(), @klaus_id, 'Quantum Optics, Photonics', 'Max Planck Institute', 'active'),
(NEWID(), @jennifer_id, 'Natural Language Processing, Deep Learning', 'Google Research', 'active'),
(NEWID(), @robert_id, 'Infectious Diseases, Antiviral Therapies', 'NIH', 'active'),
(NEWID(), @james_id, 'Statistics, Climate Modeling', 'University of Oxford', 'active'),
(NEWID(), @anna_id, 'Genetics, Molecular Biology', 'ETH Zurich', 'active'),
(NEWID(), @pierre_id, 'Microbiology, Virology', 'Pasteur Institute', 'active'),
(NEWID(), @maria_id, 'Aerospace Medicine, Space Research', 'NASA', 'active');

-- 22. Insertar procesos de evaluación
INSERT INTO review_processes (publication_id, reviewer_id, assignment_date, deadline_date, comments, decision, score)
SELECT @pub1_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @sarah_id), '2024-02-01', '2024-03-01', 'Well-structured and novel approach.', 'accepted', 5
UNION ALL
SELECT @pub2_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @michael_id), '2024-02-10', '2024-03-10', 'Strong methodology but lacks scalability discussion.', 'minor_revision', 4
UNION ALL
SELECT @pub3_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @emily_id), '2024-03-05', '2024-04-05', 'Excellent translational potential.', 'accepted', 5
UNION ALL
SELECT @pub4_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @klaus_id), '2024-02-15', '2024-03-15', 'Solid experimental setup, minor clarifications needed.', 'minor_revision', 4
UNION ALL
SELECT @pub5_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @jennifer_id), '2024-03-01', '2024-04-01', 'Innovative use of transformers, recommend acceptance.', 'accepted', 5
UNION ALL
SELECT @pub6_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @robert_id), '2024-02-20', '2024-03-20', 'Comprehensive antiviral analysis, high relevance.', 'accepted', 5
UNION ALL
SELECT @pub7_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @james_id), '2024-01-30', '2024-02-28', 'Good statistical depth, minor data source issues.', 'minor_revision', 4
UNION ALL
SELECT @pub8_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @anna_id), '2024-02-25', '2024-03-25', 'Excellent demonstration of precision gene editing.', 'accepted', 5
UNION ALL
SELECT @pub9_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @pierre_id), '2024-03-10', '2024-04-10', 'Impressive robotics application, needs clarity in safety protocols.', 'major_revision', 3
UNION ALL
SELECT @pub10_id, (SELECT reviewer_id FROM reviewers WHERE researcher_id = @maria_id), '2024-02-18', '2024-03-18', 'Promising nanomaterial research, some reproducibility concerns.', 'pending', 1;

-- 23. Insertar relación revisores-áreas
...
((SELECT reviewer_id FROM reviewers WHERE researcher_id = @robert_id), @med_area_id, 'medium'),
((SELECT reviewer_id FROM reviewers WHERE researcher_id = @james_id), @stats_area_id, 'high'),
((SELECT reviewer_id FROM reviewers WHERE researcher_id = @anna_id), @genetics_area_id, 'high'),
((SELECT reviewer_id FROM reviewers WHERE researcher_id = @pierre_id), @bio_area_id, 'medium'),
((SELECT reviewer_id FROM reviewers WHERE researcher_id = @maria_id), @phy_area_id, 'medium');
...
VALUES
(@sarah_id, @pub1_id, 'citations', 42, '2024-03-01', 'Scopus'),
(@michael_id, @pub2_id, 'citations', 28, '2024-03-01', 'Web of Science'),
(@emily_id, @pub3_id, 'citations', 35, '2024-03-01', 'Scopus'),
(@klaus_id, @pub4_id, 'citations', 19, '2024-03-01', 'Web of Science'),
(@jennifer_id, @pub5_id, 'citations', 31, '2024-03-01', 'Scopus'),
(@robert_id, @pub6_id, 'citations', 57, '2024-03-01', 'Web of Science'),
(@james_id, @pub7_id, 'citations', 23, '2024-03-01', 'Scopus'),
(@sarah_id, @pub8_id, 'citations', 48, '2024-03-01', 'Web of Science'),
(@michael_id, @pub9_id, 'citations', 26, '2024-03-01', 'Scopus'),
(@emily_id, @pub10_id, 'citations', 39, '2024-03-01', 'Scopus');
